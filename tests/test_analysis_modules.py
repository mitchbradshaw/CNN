...
=========================
Comprehensive automated unit tests for every analysis function in
Working/Detection/analysis/.

Run from the project root:
    python tests/test_analysis_modules.py

Or as a module:
    python -m tests.test_analysis_modules
"""
...
# ── Path setup ────────────────────────────────────────────────────────────────
# Walk up to the directory containing Working/, so this survives future moves.
PROJECT_ROOT = os.path.dirname(os.path.abspath(__file__))
while not os.path.isdir(os.path.join(PROJECT_ROOT, "Working")) \
        and os.path.dirname(PROJECT_ROOT) != PROJECT_ROOT:
    PROJECT_ROOT = os.path.dirname(PROJECT_ROOT)

ANALYSIS_DIR = os.path.join(PROJECT_ROOT, "Working", "Detection", "analysis")
ANALYSIS_PKG = "Working.Detection.analysis"
...
def discover_modules() -> dict[str, types.ModuleType]:
    """Import every .py file in Working/Detection/analysis/ (except __init__)."""
    ...
    for fname in sorted(os.listdir(ANALYSIS_DIR)):
        ...
        if stem.startswith("__") or stem in _SKIP_MODULES:
            continue
        try:
            mod = importlib.import_module(f"{ANALYSIS_PKG}.{stem}")
            discovered[stem] = mod
