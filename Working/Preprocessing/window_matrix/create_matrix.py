...
from Working.Detection.aeon_features.transformations import add_catch22_to_matrix
from Working.Preprocessing.manage_data.load_data import load_raw_data
from Working.Detection.analysis.entropy_analysis import _shannon_entropy, _svd_entropy, _sample_entropy, _spectral_entropy, _permutation_entropy, _approximate_entropy
import numpy as np
...
def update_fusion_prediction_v1_columns(wm,modelpath,csvfilename,batchsize=64):
    from Working.Catalogue.cnn.apply_cnn import add_fusion_prediction_v1_scores
...
def encode_psax_windowed(wm, csvfilename, dim_ratio=0.1, alphabet_size=8):
    from Working.Detection.sax.sax_encoding import make_psax_encoder
...
def encode_psax_entire(wm, csvfilename, dim_ratio=0.1, alphabet_size=8, training_frac=1):
    from Working.Detection.sax.sax_encoding import encode_dataset_psax
...
def encode_csax_windowed(wm, csvfilename, dim_ratio=0.1):
    from Working.Detection.sax.sax_encoding import make_csax_encoder
...
def encode_csax_entire(wm, csvfilename, dim_ratio=0.1, training_frac=1):
    from Working.Detection.sax.sax_encoding import encode_dataset_csax
