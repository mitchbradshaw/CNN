...
handling failures gracefully and persisting outputs to test_out/.
...
from manage_data.load_data import load_raw_data   # noqa: E402 (after sys.path fix)
...
def load_dataset() -> AeonDataset:
    ...
    if USE_WIN_MATRIX:
        ...
        from matrix_calc import load_matrix   # lazy import; avoids circular deps
...
def save_output(name: str, payload: dict, summary: Optional[str] = None) -> None:
    """Persist result to test_out/<name>_output.pkl and optionally .txt."""
...
def pipeline_pr_threshold_sweep(ds: AeonDataset) -> dict:
    ...
    Saves two extra CSVs:
      test_out/pipeline_pr_threshold_sweep_discrete.csv  — per-threshold metrics
      test_out/pipeline_pr_threshold_sweep_prcurve.csv   — continuous PR curve
    """
    import csv as _csv
    from aeon.transformations.collection.feature_based import Catch22
    from sklearn.ensemble import RandomForestClassifier
