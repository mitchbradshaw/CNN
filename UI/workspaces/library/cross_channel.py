...
cross_channel.py
================
Cross-channel classification action for the Library workspace (ticket 41).

The UI half of a library-level action. The computation lives in
`Working.cross_channel` and the edge update in `Working.library`; this module
only wires a Panel button to that seam and reports how many edges were
classified. If the UI action is cut, `Working.cross_channel` still runs as a
script.

Named `cross_channel.py` rather than `detail.py`: ticket 39 owns `detail.py`
for the `EntryDetail` surface, and the two are unrelated modules that happened
to pick the same filename in parallel branches.
"""
