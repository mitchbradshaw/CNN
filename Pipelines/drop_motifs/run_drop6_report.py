...
corrected figure set, and a handoff report.
    ...
    python Pipelines/drop_motifs/run_drop6_report.py
    python Pipelines/drop_motifs/run_drop6_report.py --spans 26 385
    python Pipelines/drop_motifs/run_drop6_report.py --no-figures

Runs ALONGSIDE `run_drop5_report.py`, never over it. That script writes
`Plots/drop_motifs5/`; this one writes `Plots/drop_motifs6/` and touches
nothing the previous run produced, so the two are comparable span by span
...
pass is idempotent rather than duplicating. See `passes6.motif_key`.
"""
...
DEFAULT_PLOT_DIR = os.path.join("Plots", "drop_motifs6")

STORE_KIND = "drop_motifs6"
...
def run_span(conn, catalogue_id, spec, args):
    ...
    rows, arrays, info = passes6.detect_multiscale(
        x, fs,
    ...
    base = info["passes"].get(passes6.PASS_BASE, {})
...
def main(argv=None):
    ...
    for catalogue_id in wanted:
        ...
        print(f"      n={summary['n_motifs']:<4d} pure={summary['n_pure']:<4d} "
              ...
              f"sens={kept.get('sens', 0):<3d} inv={kept.get('inv', 0):<3d}  "
    ...
    from Pipelines.drop_motifs.report6 import write_report
