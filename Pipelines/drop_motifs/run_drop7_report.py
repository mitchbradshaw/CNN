...
corrected figure set, and a handoff report.
    ...
    python Pipelines/drop_motifs/run_drop7_report.py
    python Pipelines/drop_motifs/run_drop7_report.py --spans 26 385
    python Pipelines/drop_motifs/run_drop7_report.py --no-figures

Runs ALONGSIDE the 5 and 6 runs, never over them. This one writes
`Plots/drop_motifs7/` and touches nothing either previous run produced, so the two are comparable span by span
...
DEFAULT_PLOT_DIR = os.path.join("Plots", "drop_motifs7")

STORE_KIND = "drop_motifs7"
