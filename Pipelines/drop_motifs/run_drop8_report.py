...
corrected figure set, and a handoff report.
    ...
    python Pipelines/drop_motifs/run_drop8_report.py
    python Pipelines/drop_motifs/run_drop8_report.py --spans 26 385
    python Pipelines/drop_motifs/run_drop8_report.py --no-figures

drop_motifs8 runs the full catalogue - all sixteen spans - with the 7.3
figure set plus four changes.

  A RISE IS ONLY WHAT THE DROPS LEFT OVER. `passes8` adds the mirror of
  the leading-edge rule: an inverted detection whose ONSET sits at a kept
  drop's TROUGH is that drop's recovery edge. Measured on the 7.3 store,
  that is every one of ID 10's thirty rises and all four of ID 22's.

  A RISE IS DRAWN AS A RISE. Inverting is what the detector does; the
  figures were inverting as well and showing rises upside down.

  THE LARGEST DROPS GET THEIR OWN FAMILY. Duration bands cannot separate
  by size, so ID 10's two large spikes sat in two different bands, each
  lumped with the small train.

  A PANEL IS ONLY AS TALL AS ITS SHAPE CLAIM EARNS. ID 385's motifs are
  202:1; drawn at the old 6:1 cap they were unreadable AND 34x from the
  truth. The cap now falls as the truth recedes.

Writes `Plots/drop_motifs8/` and touches nothing earlier.
...
from Pipelines.drop_motifs import passes7, passes8
...
DEFAULT_PLOT_DIR = os.path.join("Plots", "drop_motifs8")

STORE_KIND = "drop_motifs8"
