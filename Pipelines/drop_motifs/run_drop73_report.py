"""
run_drop73_report.py
=====================
The drop_motifs6 run: multi-scale detection over the sixteen spans, the
corrected figure set, and a handoff report.

    python Pipelines/drop_motifs/run_drop73_report.py
    python Pipelines/drop_motifs/run_drop73_report.py --spans 26 385
    python Pipelines/drop_motifs/run_drop73_report.py --no-figures

drop_motifs7.3 is a FIGURE-ONLY revision of drop_motifs7.2: the detection
passes are identical and the motif library it writes is the same set.
Three changes, all about reading the figures rather than making them.

  the overlay pages are now SIZED TO THEIR PANELS and each row centred,
  instead of shape-locked panels shrinking inside fixed gridspec cells -
  which on ID 22 gave five one-inch slivers adrift in a 15-inch sheet

  the dendrogram's merge-distance axis is RANKED rather than linear, so
  the root merge stops taking most of an A4 page and the family panels
  stop sitting in the bottom quarter under a field of white space

  the rose's MEAN is drawn on its own ray with its label beside it,
  rather than as a horizontal swatch in a legend in the corner

Writes `Plots/drop_motifs7.3/` and touches nothing earlier, so the sets
stay comparable span by span.
...
DEFAULT_PLOT_DIR = os.path.join("Plots", "drop_motifs7.3")

STORE_KIND = "drop_motifs7.3"
